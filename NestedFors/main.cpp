...
void main()
{
	...
	for (int i = 1;i <= 10;i++)
	{
	 ...
	 //√еометрические задачки

	int n;
	cout << "¬ведите количество здездочек: ";cin >> n;
#ifdef SQAURE
	for (int i = 0;i < n;i++)
	{
		for (int i = 0;i < n;i++)
	...
	for (int i = 0;i < n;i++)
	{
		for (int j = 0;j <= i;j++)//»ли можно j<i, но во внешнем for добавить n+1.
		{
			cout << "* ";
	...
	for (int i = 0;i <= n;i++)
	{
		for (int j = 0;j <= n;j++)
		{
			...
			}														// "\xDB\xDB" дл€ WHITE_BOX и "\х20\х20"
		}
		cout << endl;
	}
#endif // CHESS_BOARD
